...
# time1= 182000
# tr_span1 = 60000
# ch_span1=20000#本番では40000にする
# model_list1 = ['rf','svm','knn','logistic','percep']


# In[124]:

def final_answer(data,date,time,tr_span,ch_span):
    ...
    if int(passed_model_result[rf_flg])==1:#反転
        pred_answer['rf'][0] = int(not(pred_model_result['rf'][0]))
        pred_answer['rf'][1] = passed_model_result[rf_ac]
    else:
        pred_answer['rf'][0] = pred_model_result['rf'][0]
        pred_answer['rf'][1] = passed_model_result[rf_ac]

    if int(passed_model_result[svm_flg])==1:#反転
        pred_answer['svm'][0] = int(not(pred_model_result['svm'][0]))
        pred_answer['svm'][1] = passed_model_result[svm_ac]
    else:
        pred_answer['svm'][0] = pred_model_result['svm'][0]
        pred_answer['svm'][1] = passed_model_result[svm_ac]

    if int(passed_model_result[knn_flg])==1:#反転
        pred_answer['knn'][0] = int(not(pred_model_result['knn'][0]))
        pred_answer['knn'][1] = passed_model_result[knn_ac]
    else:
        pred_answer['knn'][0] = pred_model_result['knn'][0]
        pred_answer['knn'][1] = passed_model_result[knn_ac]

    if int(passed_model_result[logistic_flg])==1:#反転
        pred_answer['logistic'][0] = int(not(pred_model_result['logistic'][0]))
        pred_answer['logistic'][1] = passed_model_result[logistic_ac]
    else:
        pred_answer['logistic'][0] = pred_model_result['logistic'][0]
        pred_answer['logistic'][1] = passed_model_result[logistic_ac]

    if int(passed_model_result[percep_flg])==1:#反転
        pred_answer['percep'][0] = int(not(pred_model_result['percep'][0]))
